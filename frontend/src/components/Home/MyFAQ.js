    ...
    <div className={`MyFAQ-container ${isOpen ? "open" : ""}`}>
      <div className="MyFAQ-header">
        ...
        <button className="MyFAQ-button" onClick={toggleMyFAQ}>
          {isOpen
            ? "Hide FAQs"
            : "What, Why, Who, When, Where & How? We have got you covered!"}
        </button>
      </div>
      {isOpen && (
        <div className="MyFAQ-content">
          <div className="MyFAQ-list">
            {myFaqs.map((myFaq, index) => (
              <div
                className={`MyFAQ-item ${activeIndex === index ? "open" : ""}`}
                key={index}
              >
                <button
                  className="MyFAQ-question"
          ...
          <div className="MyFAQ-img">
            <img
              className="MyFAQ-img-img"
              src="https://miro.medium.com/v2/resize:fit:828/0*FdLLSjLPudGd-Pt5"
            />
          </div>
        </div>
      )}
